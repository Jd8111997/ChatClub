...
.controller('RoomController', ['$modal', '$scope', '$timeout', '$routeParams', '$location', '$rootScope', 'matrixService', 'eventHandlerService', 'mFileUpload', 'MatrixCall', 'modelService', 'recentsService', 'mUserDisplayNameFilter', 'dialogService', 'typingService', 'notificationService',
   ...
   'use strict';
    ...
    //-----------------for dob print--------------------------------------
       // Display dob
       console.log("system date = ",$scope.state.systemdate);
        matrixService.getdob($scope.state.user_id).then(
            function(response) {
                $scope.state.dob = response.data.dob;
                console.log("dob in room entered = ",$scope.state.dob);
                $scope.state.displayName = response.data.displayname;
                console.log("displayname in room = ",$scope.state.displayName);
                var day = $scope.state.dob.split("/");
                $scope.state.dob_date = $scope.state.dob.split("/");
                console.log("dob_date = ",$scope.state.dob_date);
                console.log("day = ",day);
                var sysday = new Date().getDate();

                var sysmonth = new Date().getMonth()+1;
                var sysyear = new Date().getFullYear();
                if (day[0]==sysday && day[1]==sysmonth){
                    $scope.state.hide_data = true;
                    console.log("hide_data = ",$scope.state.hide_data);
                    var age = sysyear-day[2];
                    $scope.state.dob = "Happy birthday to "+$scope.state.displayName+" from CHATCLUB";
                    //matrixService.sendTextMessage($scope.room_id,$scope.state.dob,$scope.state.user_id);$routeParams.room_id_or_alias
                    //msg = matrixService.bithdaywish($scope.room_id,$scope.state.dob,$scope.state.user_id);
                    matrixService.bithdaywish($routeParams.room_id_or_alias,$scope.state.dob,$scope.room_id);
                        /*
                    ...
                    setTimeout(function(){localStorage.removeItem("justOnce")}, 86400000);
                    */
                    console.log("In condition check = ",$scope.state.dob);
                }
    ...
    setTimeout(function(){localStorage.removeItem("justOnce")}, 86400000);
                ...
                */
                ...
                config.birth = response.data.dob;
                matrixService.setConfig(config);
                matrixService.saveConfig();
                console.log("config birth = %s",config.birth);*/
            },
            function(error) {
                $scope.feedback = "Can't load dob from room controller";
            }
        );
        //------------------------------------------------------------------
    ...
    $scope.topic = {
        ...
        editTopic: function() {
            ...
            var avatarEvent = $scope.room.current_room_state.state_events['m.room.avatar'];
    ...
    // Update Group icon functions added by AKSHAY
    $scope.$watch("groupIcon.avatarFile", function(newValue, oldValue) {
        if ($scope.groupIcon.avatarFile) {
            console.log("Uploading new avatar file...");
            mFileUpload.uploadFile($scope.groupIcon.avatarFile).then(
                function(url) {
                    $scope.groupIcon.avatarUrl = url;
                },
                function(error) {
                    dialogService.showError(error);
                } 
    ...
    $scope.saveGroupIcon = function() {
        console.log("inside savegroupicon");
        //if ($scope.groupIcon.avatarUrl !== $scope.groupIconOnServer.avatarUrl) {
            setAvatar($scope.groupIcon.avatarUrl);
        //}
    };

    var setAvatar = function(avatarURL) {
        console.log("Updating avatar to " + avatarURL);
        matrixService.setGroupIconUrl($scope.room_id, avatarURL).then(
            function(response) {
                $scope.groupIconOnServer.avatarUrl = avatarURL;
