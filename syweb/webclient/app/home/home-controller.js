...
.controller('HomeController', ['$scope', '$rootScope', '$location', 'matrixService', 'eventHandlerService', 'recentsService', 'dialogService', '$modal',
    ...
    $scope.dob=matrixService.getdob();
    ...
    $scope.onInit = function() {
        ...
        //-----------------for dob print--------------------------------------
       // Display dob
       matrixService.getdob($scope.config.user_id).then(
        function(response) {
            $scope.profile.dob = response.data.dob;
            $scope.profile.displayName = response.data.displayname;
            var day = $scope.profile.dob.split("/");
            console.log("day = ",day);
            ...
            if (day[0]==sysday && day[1]==sysmonth){
                ...
                $scope.profile.dob = "Happy birthday to "+$scope.profile.displayName+" Now your age is "+age;
            }

        },
        function(error) {
            $scope.feedback = "Can't load dob from home controller";
        }
    );
//------------------------------------------------------------------
