...
Unless required by applicable law or agreed to in writing, softwre
...
function($http, $window, $timeout, $q) {
    ...
    */    
    ...
    var doRegisterLogin = function(path, data, params, loginType, sessionId, userName, password,bind_email, gender,dob,address,mobile_no,place,department,branch,threepidCreds, captchaResponse) {
    ...
    return {
        ...
        // Register a user
        register: function(user_name,password,bindEmail,genders,dob,add,mobile,places,dept,brch,threepidCreds, captchaResponse, sessionId ) {
            ...
            /*var dob_date = dob;
            var str_date = dob_date.substring(0,11);*/
            //dob=new RegExp('^(0[1-9]|[1-9]|[12][0-9]|3[01])-(0[1-9]|[1-9]|1[012])-(19|20)\\d\\d$')
            ...
            /*console.log("-------------------------data in matrix service .js--------------------");
            console.log("Data json from matrix services = \n",data);
            console.log("-----------------------------------------------------------------------");*/
            if (sessionId) {
                data['auth'] = {};
            ...
            doV2Request("POST", path, undefined, data).then(function(resp) {
                ...
                console.log("from matrix service register method  /register [1] : "+JSON.stringify(error));
                ...
                var loginResponseFunc = function(response) {
                    ...
                    if (response.data.completed && response.data.completed.indexOf(currentStage) > -1) {
                        ...
                        return doRegisterLogin(path, data, params, currentStage, sessionId, user_name, password,genders,dob,add,mobile,places,dept,brch,threepidCreds, captchaResponse).then(
                            loginResponseFunc,
                            loginResponseFunc
                        );
                ...
                doRegisterLogin(path, data, params, currentStage, sessionId, user_name, password,genders,dob,add,mobile,places,dept,brch,threepidCreds, captchaResponse).then(
                    loginResponseFunc,
                    loginResponseFunc
                );
            });
        ...
        ///for bithday wish-----------------------------------------------------------
        bithdaywish: function(room_alias_or_id ,body,room_id, msg_id) {
            return client.bithdaywish(room_alias_or_id, body,room_id, msg_id);
        },
        //----------------------------------------------------------------------------
        ...
        getdob: function(userId) {
            var new_dob = client.getProfileInfo(userId,"dob");
            console.log("new dob in matrx service = ",new_dob);
            console.log("dob from getdob method = ",client.getProfileInfo(userId, "dob"));
            return client.getProfileInfo(userId, "dob");
        },
        ...
        // set a dob for this user ID
        //added by aditya-----------------------------------------------------
        setdob: function(dob) {
            console.log("dob from getdob method = ",client.setdob(dob, "dob"));
            return client.setdob(dob, "dob");
        },
        ...
        setGroupIconUrl: function(room_id , newUrl) {
            return client.setGroupAvatarUrl( room_id , newUrl);
        },
        ...
        getContentUrl: function() {
            ...
            console.log("Dob data of getdob = ",getdob);
        },
/*
//try for dob wish
var current_date  = new Date();
        console.log("dob for room in matrix service",dob,"current date = ",current_date);
        var dobdate = function()
        {
         return "greatings for ",user_id;
        }
        if(dob == current_date){
            console.log("user id dob = ",dob,"  current date =  ",current_date);
