...
ProfileInfo = namedtuple(
    "ProfileInfo", ("avatar_url", "display_name","dob")
)
    ...
    def _get_joined_users_from_context(self, room_id, state_group, current_state_ids,
            ...
            if ev_entry:
                if ev_entry.event.membership == Membership.JOIN:
                    users_in_room[to_ascii(ev_entry.event.state_key)] = ProfileInfo(
                        ...
                        dob = to_ascii(
                            ev_entry.event.content.get("dob",None)
                        ),
                    )
            else:
        ...
        if missing_member_event_ids:
            rows = yield self._simple_select_many_batch(
                ...
                retcols=('user_id', 'display_name', 'avatar_url',"dob"),
                keyvalues={
            ...
            if event.membership == Membership.JOIN:
                if event.event_id in member_event_ids:
                    users_in_room[to_ascii(event.state_key)] = ProfileInfo(
                        ...
                        dob=to_ascii(event.content.get("dob", None))
                    )
            ...
            values=[
                {
                    ...
                    "dob": event.content.get("dob", None),
                }
                for event in events
            ]
        )
    ...
    def _background_add_membership_profile(self, progress, batch_size):
        ...
        def add_membership_profile_txn(txn):
            ...
            for row in rows:
                ...
                dob = content.get("dob", None)
                if display_name or avatar_url or dob:
                    to_update.append((
                        display_name, avatar_url,dob, event_id, room_id
                    ))

            to_update_sql = ("""
                UPDATE room_memberships SET display_name = ?, avatar_url = ?, dob=?
