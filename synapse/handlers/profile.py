    ...
    def get_profile(self, user_id):
        ...
        if self.hs.is_mine(target_user):
            try:
                ...
                #--------------for dob print-----------------------
                dob = yield self.store.get_profile_dob(
                    target_user.localpart
                )
                #--------------------------------------------------
    ...
    def get_dob(self, target_user):
        if self.hs.is_mine(target_user):
            try:
                ...
                if e.code == 404:
                    raise SynapseError(404, "User dob was not found", Codes.NOT_FOUND)
                raise

            defer.returnValue(dob)
            ...
            try:
                ...
                if e.code != 404:
                    logger.exception("Failed to get dob")
                raise

                defer.returnValue(result["dob"])   

    @defer.inlineCallbacks
    def set_dob(self, target_user, requester, new_dob, by_admin=False):
        """target_user is the user whose displayname is to be changed;
        auth_user is the user attempting to make this change."""
        if not self.hs.is_mine(target_user):
            raise SynapseError(400, "User is not hosted on this Home Server")
        ...
        yield self.store.set_user_dob(
            target_user.localpart, new_dob
        )
...
##added by Jaydeeep
    ...
    """@defer.inlineCallbacks
    def set_room_avatar_url(self, room, requester, new_avatar_url, by_admin=False):

        yield self.store.set_group_avatar_url(
            room, new_avatar_url
        )
...
"""
    ...
    def on_profile_query(self, args):
        ...
        try:
            ...
            if just_field is None or just_field=="dob":
                response["dob"] = yield  self.store.get_user_dob(
                    user.localpart
                )
        except StoreError as e:
            if e.code == 404:
...
class MasterProfileHandler(BaseProfileHandler):
    ...
    def _update_remote_profile_cache(self):
        ...
        for user_id, displayname, avatar_url,dob in entries:
            ...
            try:
                ...
                yield self.store.update_remote_profile_cache(
                    user_id, displayname, avatar_url,dob
                )
                continue

            new_name = profile.get("displayname")
            new_avatar = profile.get("avatar_url")
            new_dob = profile.get("dob")
            # We always hit update to update the last_check timestamp
            yield self.store.update_remote_profile_cache(
                user_id, new_name, new_avatar,new_dob
            )
