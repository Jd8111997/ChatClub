...
class UserDirectoryHandler(object):
    ...
    def handle_local_profile_change(self, user_id, profile):
        ...
        yield self.store.update_profile_in_user_dir(
            user_id, profile.display_name, profile.avatar_url, profile.dob , None,
        )
    ...
    def _handle_profile_change(self, user_id, room_id, prev_event_id, event_id):
        ...
        pre_dob = prev_event.content.get("dob")
        new_dob = event.content.get("dob")

        if prev_name != new_name or prev_avatar != new_avatar or prev_dob != new_dob:
            yield self.store.update_profile_in_user_dir(
                user_id, new_name, new_avatar, new_dob, room_id,
            )
